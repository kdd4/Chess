...
namespace Chess
{
    namespace Pieces
    {
        class Knight : public IMovablePiece
        {
        public:
            Knight(Position pos, PieceColor color, std::weak_ptr<Board>& board);
            Knight(const Piece& piece, std::weak_ptr<Board> board);
            Knight(const MovablePiece& right);
